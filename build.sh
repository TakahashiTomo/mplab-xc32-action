...
[[ -d "$NB_DIR" ]] || { echo "Error: nbproject missing"; exit 1; }
[[ -f "$ROOT_MK" ]] || { echo "Error: root Makefile missing"; exit 1; }
...
for f in ${NB_DIR}/*.mk; do
  # SHELL=cmd.exe → SHELL=/bin/sh
  sed -i 's/^SHELL=.*/SHELL=\/bin\/sh/' "$f"

  # COMSPEC/ComSpec → コメントアウト
  sed -i 's/^COMSPEC=.*/# COMSPEC disabled in CI/' "$f"
  sed -i 's/^ComSpec=.*/# ComSpec disabled in CI/' "$f"
  sed -i 's/^MP_SHELL=.*/# MP_SHELL disabled in CI/' "$f"

  # Windows style PATH → 置換
  sed -i "s|^PATH:=.*|PATH:=${XC32_BIN}:\$(PATH)|" "$f"

  # XC32 コンパイラパスを Linux へ変更
...
echo "=== DEBUG: head of Makefile-${CONFIG}.mk ==="
sed -n '1,160p' "${NB_DIR}/Makefile-${CONFIG}.mk"
...
# -----------------------------------------------------------------------------
# 5) Build（root Makefile を使う）
# -----------------------------------------------------------------------------
echo "=== Building with make (root Makefile) ==="
make --trace --no-builtin-rules -C "${PROJECT_DIR}" CONF="${CONFIG}" build
