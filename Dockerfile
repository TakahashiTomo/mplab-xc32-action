...
# -----------------------------------------------------
# APT ミラー切替 + リトライ + 必要パッケージインストール
# （try_update / try_install はこの RUN 内でのみ有効）
# -----------------------------------------------------
RUN set -eux; \
    try_update() { \
      for i in 1 2 3; do \
        apt-get update -qq && return 0 || sleep 5; \
      done; return 1; \
    }; \
    try_install() { \
      for i in 1 2 3; do \
        apt-get install -y -qq --no-install-recommends "$@" && return 0 || sleep 5; \
      done; return 1; \
    }; \
    # ミラー切替
    if ! try_update; then \
    ...
    # 最小構成ツール
    try_install wget tar xz-utils libusb-1.0-0 make gcc; \
    apt-get clean; rm -rf /var/lib/apt/lists/*

# -----------------------------------------------------
# 作業ディレクトリ（InstallAnywhere巨大展開対策）
# -----------------------------------------------------
